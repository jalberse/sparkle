...
#define SHORTCUT_BOOL(key, variable) if (Global::input->GetKeyDown(key)) variable = !variable
	...
	struct SolverTiming
	{
		...
		std::unordered_map<std::string, double> label2time;
		std::unordered_map<std::string, double> label2avgTime;

		// Colors for high/mid/low percentages
		...
		ImVec4 color_disabled = ImVec4(0.5f, 0.5f, 0.5f, 1.0f);

		void DisplayKernelTiming(const std::string name, bool autoColor = true)
		{
			double total = label2avgTime["Total"];
			bool shouldPop = false;
			float percentage = (total > 0) ? float(label2avgTime[name] / total * 100.0) : 0.0f;
			if (autoColor)
			{
		...
		void Update()
		{
			if (Timer::PeriodicUpdate("GUI_SOLVER", 0.2f))
			{
				...
				for (const auto& label : labels)
				{
					...
					{
						label2time["kernelSum"] += label2time[label];
					}
				}
		...
		void OnGUI()
		{
			if (ImGui::CollapsingHeader("Solver timing"))
			{
	...
	struct PerformanceStat
	{
		...
		double cpuTime = 0.0;
		// includes solver_time + cuda_sync_time
		double gpuTime = 0.0;
		...
		void Update()
		{
			...
			float deltaTimeMiliseconds = Timer::deltaTime() * 1000.0f;
			...
			if (Timer::PeriodicUpdate("GUI_SLOW", 0.3f))
			{
				...
				cpuTime = Timer::GetTimer("CPU_TIME") * 1000.0;
				gpuTime = Timer::GetTimer("GPU_TIME") * 1000.0;
		...
		void OnGUI()
		{
			if (ImGui::BeginTable("stat", 2, ImGuiTableFlags_SizingStretchProp))
			{
				...
				ImGui::TableNextColumn(); ImGui::Text("Render Frame Rate: ");
			...
			ImGui::PlotLines("##", graphValues, IM_ARRAYSIZE(graphValues), graphIndex, overlay.c_str(), 0, graphAverage * 2.0f, ImVec2(0, 80.0f));
			ImGui::Dummy(ImVec2(0, 5));
		}
	};
	...
	void GUI::RegisterDebugOnce(const std::string& debugMessage)
	{
	...
	GUI::GUI(GLFWwindow* window)
	{
		...
		io.IniFilename = NULL;
		io.Fonts->AddFontFromFileTTF("Assets/DroidSans.ttf", 19.0f);
		//io.ConfigFlags |= ImGuiConfigFlags_NavEnableKeyboard;     // Enable Keyboard Controls
		//io.ConfigFlags |= ImGuiConfigFlags_NavEnableGamepad;      // Enable Gamepad Controls

		CustomizeStyle();

		// Setup platfom/renderer backends
	...
	void GUI::ShowOptionWindow()
	{
		...
		ImGui::SetNextWindowPos(ImVec2(20, 50 + (m_canvasHeight - 60.0f) * 0.4f));
	...
	void GUI::ShowStatWindow()
	{
		...
		ImGui::SetNextWindowPos(ImVec2(m_canvasWidth - k_rightWindowWidth * 1.1 - 20.0, 20.0));
